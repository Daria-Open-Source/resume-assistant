...
export default class Scheduler {
    ...
    scheduleJobs() {
        ...
        this.jobs.map(job => cron.schedule('0 * * * *', async () => await job()));
    }
    ...
    _validateList(list) {
        ...
        if (!list.every(job => typeof job == typeof Function))
            throw new Error('Every job must be a function referencing an update method');
    }
};
